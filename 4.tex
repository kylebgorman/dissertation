%\chapter{Structural and accidental gaps\protect\symbolfootnote[1]{An abridged version of this chapter was presented at the 30th West Coast Conference on Formal Linguistics and will appear in the proceedings.}} \label{clusters}
...
\section{The static phonotactics hypothesis and the lexicon}
...
A more sophisticated approach to problems of this type, making no reference to \citeauthor{Bloomfield1930}, was published only two years later. \citet{Jakobson1932}, then a member of the Prague circle,     
...
% Pierrehumbert informal judgements * \citet{Pierrehumbert1994} and \citet{Hammond1999a} technique is not replicable, so we turn to CELEX
% not replicable, either the experience nor the data itself
% Some of Pierrehumbert's and some of Hammond's are ridiculous
% Despite this, we preserve some Pierrehumbertian generalizations

Beyond this, there is a potential for bias or circularity of argumentation. As mentioned above (\S\ref{pike}), a sequence which is rare in monomorphs is sometimes said to mark a juncture even in the absense of other evidence for segmentation. But it is certainly incorrect to use this principle to argue for a juncture if the argument is only being made to preserve this very phonotactic generalization, as \citet[186]{Pierrehumbert1994} does with the word \emph{antler}. 

% Compounds, regulars, and more productive derivatives are pretty clear 
% Latin is less clear: \citet{Bolinger1961} on stress, and \emph{SPE} and \citet[11--13]{Aronoff1976} on forms of prefixes and bound stems
%There has been a good deal of attention paid to Latinate prefixes
%\citet{Bolinger1961} suggests this for English stress.
%\citet{SPE}
...
% Harley2009 \citet{Gropen1989} \citet[56]{Coppock2008} observes that there is every reason to believe that the ditransitive is productive in the sense that it can be applied to new words; consider recent conversion \emph{text} (e.g., \emph{text me your number}). 
...
% While this is precisely the abstractness that  ???, it is reasonably well supported by psycholinguistic results in lexical decision research. 

% description of taft model \citet{Taft1975} propose an influential model of morphological processing. The first step in the \citeauthor{Taft1975} model is to strip affixes from possible roots. Root and affix are then looked up in lexical memory. At the final stage, they are recombined. 
% It has been suggested since then that full decomposition is only one of two ways to recognize complex words, the other being a whole word storage route running in cascade \citep{Caramazza1988} or in parallel \citep{Baayen1997b}. Whatever evidence is amassed for this (see \citealp{Lignos2012} for a critical review) but few if any lexical decision researchers would deny the possibility of \citep{Butterworth1983}.

% first experiment \citet{Manelis1977} who suggest poor stimulus controls \citeauthor{Taft1975}

% that *\emph{devive} in the Latinate prefix \emph{de-} applies to a bound stem \emph{vive} (e.g., \emph{revive}) is slower to be recognized than *\emph{debel}, since \emph{bel} is not recognized as abound stem in words like \emph{rebel}.
...
%\citet{Taft2004b}
%\emph{virus} is primed by \emph{viral}
...
It is not trivial in all cases to separate interludes from the nuclei on either side. Immediately below, distributional heuristics are used to develop a subsyllabic parsing procedure for English.
...
%In fact, speakers judge words in which the front onglide is preceded by an onset consonant and followed by a vowel other than [uː] to be anomalous \citep{Moreland2009}. 
